...
const GeometryScene = ({
  ...
  const [parsedGeometry, setParsedGeometry] = useState<ParsedGeometry | null>(
    null
  );
  ...
  return (
    <group>
      ...
      {parsedGeometry.parts.map((part, partIndex) => (
        <group
          key={`part-${partIndex}`}
          userData={{ partId: part.id, partName: part.name }}
        >
          {/* Render face meshes */}
...
const Experience = () => {
  ...
  // Keyboard controls
  const frontShortcutKey = useKeyboardControls(
    (state) => state[controls.FRONT]
  );
  ...
  const rightShortcutKey = useKeyboardControls(
    (state) => state[controls.RIGHT]
  );
  const topShortcutKey = useKeyboardControls((state) => state[controls.TOP]);
  const bottomShortcutKey = useKeyboardControls(
    (state) => state[controls.BOTTOM]
  );
  const orthoShortcutKey = useKeyboardControls(
    (state) => state[controls.ORTHO]
  );
  const transformShortcutKey = useKeyboardControls(
    (state) => state[controls.TRANSFORM]
  );
  ...
  useEffect(() => {
    const fitToScreenFunc = () => {
      ...
      const fov = useOrtho
        ? 1
        : (perspectiveRef.current?.fov || 50) * (Math.PI / 180);
      let cameraZ = useOrtho
        ? maxDim * 2
  ...
  const [pivotDataPreviousIndex, setPivotDataPreviousIndex] =
    useState<number>(0);
      ...
      pivotData.forEach((pivot, index) => {
        ...
        const dist =
          pivot.name === "corner"
            ...
            : pivot.name === "edge"
            ? hitPoint.distanceTo(worldPos) + 0.25
